...
PCAP_SPAM_FILES = [
    os.path.join(PCAP_DIR, "capture201108%s.csv" % i)
    for i in ["10", "11", "15-2", "17", "15-3"]
]
PCAP_DDOS_FILES = [
    os.path.join(PCAP_DIR, "capture201108%s.csv" % i) for i in ["15", "18", "18-2"]
]
PCAP_IRS_FILES = [
    os.path.join(PCAP_DIR, "capture201108%s.csv" % i)
...
def generate_pcap_agg_scenario(scenario, window_size):
    ...
    features_df = [
        _aggregate_pcap(pd.read_csv(i, parse_dates=[0]), window_size)
...
def _aggregate_pcap(packets, window_size):
    ...
    for window in tqdm(range(first_window, last_window + 1)):
        window_df = df[df['start_window'] == window]
        results.append(_aggregate_pcap_packets.remote(window))
    ray.shutdown()
...
def _aggregate_pcap_packets(packets):
    ...
    fs["n_abnormal"] = len([0 for i in packets if i[14] in MALICIOUS])
    fs["n_normal"] = len([0 for i in packets if i[14] in NORMAL])
    ...
    fs["n_tcp"] = len([0 for i in packets if int(i[12]) == 6])
    fs["n_udp"] = len([0 for i in packets if int(i[12]) == 17])
    fs["n_icmp"] = len([0 for i in packets if int(i[12]) == 1])
